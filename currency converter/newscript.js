...
const updateExchangeRate = async () => {
  ...
  // Use local Express backend endpoint for development
  const URL = `http://localhost:3000/convert?from=${fromCurr.value}&to=${toCurr.value}&amount=${amtVal}`;
  try {
    ...
    msg.innerText = `Error: Could not fetch conversion. Please check your backend.`;
    console.error(err);
  }
};
